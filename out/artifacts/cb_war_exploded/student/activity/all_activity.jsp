...
<form method="post" action="search_activity.jsp" target="right">
...
<a href="#" class="list-group-item"><%=activity.getAname()%>
    ...
    <input type="hidden" name="Cname" value="<%=activity.getCname()%>">
    <button type="button" class="btn btn-primary btn-default" data-toggle="modal" data-target="<%="#"+i%>" style="float: right">
        详情
    </button>
    <button type="submit" id="<%=i+100%>" class="btn btn-primary btn-default" name="join_bad" style="float: right">
        申请加入
    </button>
</a>
</form>
...
<script>
    document.getElementById("<%=i+100%>").onclick=function () {
        var flag=confirm("确定加入该活动吗?");
        if(!flag){
            return false;
        }
